...
public class MyGame extends VariableFrameRateGame
{
	...
	private GameObject robot, childNeptune, neptune, childJupiter, jupiter, childSaturn, saturn, childEarth, earth, x, y, z,terr;
	private ObjShape sphereS, robS, linxS, linyS, linzS, terrS, ghostS;
	private TextureImage robottx, earthx, saturnx, jupiterx, neptunex, doltx, hills,grass,desert, ghostT;
	private Light light;
	...
	{	
		robS = new ImportedModel("myRobot.obj");
		ghostS = new Sphere();
	...
	{	
		robottx = new TextureImage("myRobot.jpg");
	...
	{	
		...
		initialTranslation = initialTranslation = (new Matrix4f()).translation(-1f,1f,1f);
	...
	{	prevTime = System.currentTimeMillis();
		...
		scriptFile = new File("assets/scripts/init.js");
		this.runScript(scriptFile);
		robotLife = ((Double)(jsEngine.get("robotLife"))).floatValue();
		...
		orbitController = new CameraOrbit3D(camera, robot, controllers, engine);
	...
	{	
		...
		String dispStr2 = "Avatar position = "
			...
			+ (robot.getWorldLocation()).x()
			+ ", " + (robot.getWorldLocation()).y()
			+ ", " + (robot.getWorldLocation()).z();
	...
	/**	if(dolphinLife <= 0.0f && itemCount < maxItemCnt) 
		...
		if(robotLife <= 0.0f && itemCount < maxItemCnt) 
		{
			System.out.println("YOU LOST!");
			super.shutdown();
			System.exit(0);
		...
		Vector3f loc = robot.getWorldLocation();
		float height = terr.getHeight(loc.x(), loc.z());
		 robot.setLocalLocation(new Vector3f(loc.x(), height, loc.z()));
	...
	*/
	...
	{
		Vector3f avaLoc = robot.getWorldLocation();
	...
	public GameObject getAvatar() { return robot; }
	...
	{ 
		...
		this.robot.setLocalTranslation(translation); 
	}
	...
	public Vector3f getPlayerPosition() { return robot.getWorldLocation(); }
}